  ...
  try {
    ...
    return unstable_cache(
      async () => {
        ...
        // 1. Surplus Alert
        const { data: monthlyData } = await innerSupabase
        ...
        const { data: savingsData } = await innerSupabase
      ...
      [`notifications-${user.id}-${currentYear}-${currentMonth}`],
      { tags: [`user-${user.id}`], revalidate: 30 }
    )();
  } catch (error) {
